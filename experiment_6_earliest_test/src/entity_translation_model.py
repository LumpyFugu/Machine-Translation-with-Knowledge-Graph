...
# === OOB hard check (print via exception message) ===
        ...
        if mx_src >= num_src_emb:
            ...
            logger.warning("[SRC OOB] num_emb=%s, bad_count=%s", num_src_emb, bad.size(0))
            logger.warning("[SRC OOB] first_bad_positions=%s", bad[:10].tolist())
            # If possible, also print a short token snippet from that sample
            bi, ti = bad[0].tolist()
            logger.warning("[SRC OOB] sample[%s] head tokens=%s", bi, src_tokens[bi][:50].tolist())
            raise RuntimeError(
...
# ==========================================
        ...
        # avoid forwarding src_ne to decoder (decoder does not accept this arg)
        ...
        if kg_embedding is not None and not hasattr(self, "_printed_kg"):
            ...
            logger.warning("[DEBUG] pad_id=%s kg_pad_norm=%s", pad_id, float(self.kg_table[pad_id].norm()))
            logger.warning("[DEBUG] kg_table=%s", tuple(self.kg_table.shape))
            logger.warning(
                "[DEBUG] kg_embedding=%s dtype=%s device=%s",
                tuple(kg_embedding.shape),
                kg_embedding.dtype,
                kg_embedding.device,
            )
            logger.warning(
                "[DEBUG] src_tokens range: %s %s", int(src_tokens.min()), int(src_tokens.max())
            )
        ...
        if src_ne is not None and not hasattr(self, "_printed_src_ne"):
            ...
            logger.warning("[DEBUG] src_ne: %s %s %s", src_ne.shape, int(src_ne.min()), int(src_ne.max()))
    ...
    def build_model(cls, args, task):
        ...
        if getattr(args, "kg_embed_path", None):
            ...
            if kg.size(0) < vocab_size:
                ...
                logger.warning("| [kg] padded %s rows to match src_dict: %s", pad_rows, kg.size(0))
            ...
            # normalize each kg_table row first (padded rows remain 0)
            kg = kg / (kg.norm(dim=1, keepdim=True) + 1e-8)

            model.register_buffer("kg_table", kg, persistent=False)
            # also place kg_table on encoder so SequenceGenerator->encoder path can access it directly
            model.encoder.register_buffer("kg_table", kg, persistent=False)
        else:
            model.kg_table = None
            model.encoder.kg_table = None
