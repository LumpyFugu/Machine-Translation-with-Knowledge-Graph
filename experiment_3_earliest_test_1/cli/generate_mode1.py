...
def main(args):
    ...
    # Compatible with newer fairseq CLI where some args may be absent
    if not hasattr(args, "max_sentences"):
        args.max_sentences = None
    if not hasattr(args, "max_tokens"):
        args.max_tokens = None
    if not hasattr(args, "remove_bpe"):
    ...
    # Generate and compute BLEU score (compatible with new fairseq scoring API)
    if args.sacrebleu:
        ...
        for sample in t:
            ...
            for i, sample_id in enumerate(sample['id'].tolist()):
                ...
                if not args.quiet:
                    ...
                    print('SEP-{}\t{}'.format(sample_id, src_ne_str))

                    source_ne_tokens = sample['ne_source'][i,:][sample['net_input']['src_tokens'][i, :] != src_dict.pad()]
                ...
                for j, hypo in enumerate(hypos[i][:args.nbest]):
                    ...
                    if not args.quiet:
                        ...
                        print('TEP-{}\t{}'.format(sample_id, tgt_dict.ne_dict.string(hypo['tgt_ne_tokens'])))

                        target_ne_tokens = sample['ne_target'][i]
                        target_ne_tokens = target_ne_tokens[target_ne_tokens != tgt_dict.pad()]
                        print('TE-{}\t{}'.format(sample_id, tgt_dict.ne_dict.string(target_ne_tokens, args.remove_bpe, escape_unk=True)))
