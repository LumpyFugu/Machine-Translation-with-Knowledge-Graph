...
def combine_ne_with_text(tokens, entities, dictionary, max_ne_id):
    ...
    combined_tokens = []
    lang_vocab = len(dictionary.lang_dict)     # e.g. 18056
    ne_vocab   = len(dictionary.ne_dict)       # e.g. 24
    nspecial   = dictionary.ne_dict.nspecial   # count of <pad>, <bos>, <unk>, <eos>, etc.

    for i, (ne, align) in enumerate(zip(combined_ne, alignment)):
        # 1) Out-of-range or special symbol -> treat as non-entity and keep original tokens
        is_oob       = (ne < 0) or (ne >= ne_vocab)
        is_special   = (0 <= ne < nspecial)    # never treat <pad>/<bos>/<unk>/<eos> as entities
        is_O         = (not is_oob) and (dictionary.ne_dict[ne] == 'O')

        if is_oob or is_special or is_O:
            # non-entity: expand original piece
            combined_tokens.extend(tokens[align])
            if is_oob:
                # print a lightweight log to help locate data issues (first few tokens only)
                head = list(tokens[align])[:10]
                print(f"[combine_ne_with_text] skip OOB NE={ne} (ne_vocab={ne_vocab}), head={head}")
        else:
            # valid entity: map to high-id segment (ne is guaranteed < ne_vocab here)
            combined_tokens.append(ne + lang_vocab)

    return combined_tokens, combined_ne, alignment

def extract_ne_from_text(tokens, entities, ne_dict, need_type=False, return_pos=False):  # entity extraction
    """
